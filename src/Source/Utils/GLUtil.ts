...
class shaderObject {
	type;
	text;
	constructor(typeIn, textIn) {
		this.type = typeIn; //初始化type成员变量
		this.text = textIn; //初始化text成员变量
	}
}
export function initWebGLCanvas(canvasName) {
	let canvas: HTMLCanvasElement = document.querySelector('canvas');//获取Canvas对象
	var context = canvas.getContext('webgl2', { antialias: true });//获取GL上下文
	return context;//返回GL上下文对象
}
//加载单个着色器的方法
export function loadSingleShader(ctx, shaderScript) {
...
//加载链接顶点、片元着色器的方法
export function loadShaderSerial(gl: WebGL2RenderingContext, vshader, fshader) {
	//加载顶点着色器
	vshader=new shaderObject('vertex',vshader);
	fshader=new shaderObject('fragment',fshader)
